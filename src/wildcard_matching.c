...
char *
translate_pattern(char *orgp)
{
    int i;
    int sz = strlen(orgp);
    char *res = leet_malloc((sz+1)*2);
    char *p = res;
    for (i=0; i<sz; i++) {
        if (orgp[i] == '*') {
            *p++ = '.';
...
isMatch(char* s, char* p)
{
    ...
    char *ss = NULL;
    int end_flag = 0;
    pp = translate_pattern(p);
    printf("trans pattern is %s\n", pp);
    statm *stat_head = compile_pattern(pp);
...
{
    printf("isMatch result is %d\n", isMatch("acdcb","a*c?b"));//false
    printf("isMatch result is %d\n", isMatch("adceb","*a*b"));//false
    return 0;
}
