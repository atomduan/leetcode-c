...
static leet_queue *
leet_combination_sum_ii(int* candidates, int candidatesSize, int target, int *used_map)
{
    leet_queue *res=NULL;
    ...
    for (i=0; i<candidatesSize; i++) {
        if (used_map[i] > 0) continue;
        used_map[i] = 1;//set used context
        resq = NULL;
        curr_val = candidates[i];
        ntarget = target - curr_val;
        if (ntarget > 0) {
            resq = leet_combination_sum_ii(candidates,candidatesSize,ntarget,used_map);
            if (resq != NULL) {
        ...
        used_map[i] = 0;//reset used context
    }
    return res;
}
               ...
               int** columnSizes, int* returnSize) 
{
    ...
    leet_queue *resq, *r;
    used_map = leet_malloc((candidatesSize+1)*sizeof(int));
    resq = leet_combination_sum_ii(candidates,candidatesSize,target,used_map);
...
main(int argc, char **argv)
{
    ...
    res = combinationSum2(candidates,candidatesSize,target,&columnSizes,&returnSize);
    if (res != NULL) {
        printf("res is : {\n");
